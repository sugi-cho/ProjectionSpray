...
namespace Osc
{
    public class MessageEncoder
    {
        ...
        private LinkedList<object> _rawdata;
        public LinkedList<object> rawdata { get { return _rawdata; } }
        public string path { get { return _address; } }

        public MessageEncoder(string address)
        {
            _address = address;
            _params = new LinkedList<IParam>();
            _rawdata = new LinkedList<object>();
        }

        public void Add(int content) { _params.AddLast(new Int32Param(content)); _rawdata.AddLast(content); }
        public void Add(float content) { _params.AddLast(new Float32Param(content)); _rawdata.AddLast(content); }
        public void Add(string content) { _params.AddLast(new StringParam(content)); _rawdata.AddLast(content); }
        public void Add(byte[] content) { _params.AddLast(new BlobParam(content, 0, content.Length)); _rawdata.AddLast(content); }
        public void Add(byte[] content, int offset, int length) { _params.AddLast(new BlobParam(content, offset, length)); _rawdata.AddLast(content); }
